...
for train_index, test_index in kfold.split(image_indices):
	...
	with strategy.scope():
		...
		model = Segnet(input_filters=32, height=img_height, width=img_width, n_channels=3)
	...
	best_model = Segnet(input_filters=32, height=img_height, width=img_width, n_channels=3)
