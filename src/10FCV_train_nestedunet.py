...
for train_index, test_index in kfold.split(image_indices):
	...
	with strategy.scope():
		...
		model = NestedUNet(input_filters=32, height=img_height, width=img_width, n_channels=3)
	...
	best_model = NestedUNet(input_filters=32, height=img_height, width=img_width, n_channels=3)
